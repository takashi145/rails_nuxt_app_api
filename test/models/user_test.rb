...
class UserTest < ActiveSupport::TestCase
  ...
  test "name_validation" do
    # 入力必須バリデーション
    user = User.new(email: "test@testaa.com", password: "password")
    user.save
    ...
    name = "a" * max
    user.name = name
    # User.count(第一引数)の値が、1(第二引数)の数だけ増加しているか
  ...
  test "active_user_uniqueness" do
    ...
    # アクティブユーザーがいない場合に同じemailを保存できるか
    count = 3
